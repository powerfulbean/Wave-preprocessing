...
if ~exist([Targetdir_bin]) 
    mkdir([Targetdir_bin])         % 若不存在，在当前目录中产生一个子目录‘Figure’
end 
Datadir = '..\data\';
Seg_length_s = 1;%以1s为单位切割
Fs = 256; %Sample rate is 256
[~,~,Tag] = xlsread('数据标签1.3.xlsx','ADD'); 
            ...
            for p = 1:length(Seg_Channel)
                ...
                for q=1:length(tempChannel_list)
                  ...
                  if Flag_OUTPUTbin == 1
                        ...
                        str41=list{tempChannel_list(q),1};
                        fid=fopen([Targetdir_bin,'TestData17105050/','110_',str11,'_',[str311,'_',str312,'_',str313,],'Channel_',str41,'(0).txt'],'wb');
